...
-- Preserve established material draft ownership. Opening or saving an existing
-- material draft must not silently transfer ownership to the current actor.
...
begin
    ...
    if existing.status = 'draft'
       and existing.draft_session_id = target_session_id
       and (
           existing.owner_user_id is not null
           or nullif(btrim(coalesce(existing.owner_device_id, '')), '') is not null
       ) then
        draft_owner_user_id := existing.owner_user_id;
        draft_owner_device_id := nullif(btrim(coalesce(existing.owner_device_id, '')), '');
    else
        select shot.updated_by
        into draft_owner_user_id
        from public.shots shot
        where shot.session_id = target_session_id
          ...
          and shot.updated_by is not null
        order by shot.created_at asc nulls last, shot.updated_at asc nulls last, shot.id
        limit 1;

        if draft_owner_user_id is not null then
            draft_owner_device_id := null;
        else
            select event_row.actor_user_id
            into draft_owner_user_id
            from public.session_events event_row
            where event_row.org_id = target_org_id
              and event_row.property_id = target_property_id
              and event_row.session_id = target_session_id
              and event_row.event_type in ('session.started', 'session.locked')
              and event_row.actor_user_id is not null
            order by event_row.created_at asc, event_row.id
            limit 1;
            ...
            if draft_owner_user_id is not null then
                ...
                if draft_session.locked_by_user_id is not null then
                    draft_owner_device_id := nullif(btrim(coalesce(draft_session.locked_by_device_id, '')), '');
                elsif draft_session.updated_by is not null then
                    draft_owner_device_id := null;
                else
                    draft_owner_device_id := nullif(btrim(coalesce(target_device_id, '')), '');
                end if;
            end if;
        end if;
    end if;
...
begin
    ...
    if draft_session.id is not null then
        ...
        into draft_owner_user_id
        from public.shots shot
        where shot.session_id = draft_session.id
          and shot.org_id = target_org_id
          and shot.deleted_at is null
          and coalesce(shot.lifecycle_state, 'active') not in ('retired', 'deleted')
          and coalesce(shot.hidden_from_gallery, false) = false
        ...
        if draft_owner_user_id is not null then
            ...
            where event_row.org_id = target_org_id
              and event_row.property_id = target_property_id
              and event_row.session_id = draft_session.id
              and event_row.event_type in ('session.started', 'session.locked')
              and event_row.actor_user_id is not null
            order by event_row.created_at asc, event_row.id
            limit 1;
