...
final class DetailTypesModel: ObservableObject {
...
struct ContentView: View {
    ...
    struct FlaggedHistoryDisplayEvent: Identifiable {
        let id: String
        let title: String
        let timestamp: Date
    ...
    static func conciseContextLabel(building: String?, elevation: String?, detailType: String?) -> String {
    ...
    static func observationCurrentReasonText(_ observation: Observation) -> String? {
    ...
    static func formatObservationHistoryTimestamp(_ date: Date) -> String {
        let formatter = DateFormatter()
    ...
    static func reportAsset(from localIdentifier: String?) -> ReportAsset? {
        let trimmed = localIdentifier?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
    ...
    static func reportAsset(fromPath path: String) -> ReportAsset? {
    ...
    static func requestUbiquitousDownloadIfNeeded(path: String) {
        let trimmed = path.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return }
        let url = URL(fileURLWithPath: trimmed)
...
extension ContentView {
    ...
    struct ManageDetailTypesView: View {
    ...
    struct ManageBuildingsSheet: View {
        @Environment(\.colorScheme) private var colorScheme
        private var theme: SheetControlTheme { .forScheme(colorScheme) }
