...
final class Phase2C25BSnapshotRestoreDiagnosticsTests: XCTestCase {
    ...
    private func localTestSecretValue(for key: String) -> String? {
        ...
        case "SCOUTCAPTURE_RUN_26P_STAGING_CANONICAL_OVERLAY_VALIDATION":
            jsonKey = "run_26p_staging_canonical_overlay_validation"
        ...
        case "SCOUTCAPTURE_26N_STAGING_SERVICE_ROLE_KEY":
            jsonKey = "staging_service_role_key"
        case "SCOUTCAPTURE_26P_STAGING_SERVICE_ROLE_KEY":
            jsonKey = "staging_service_role_key"
        default:
            return nil
        }
    ...
    func testPhase2C26PHostedStagingCanonicalCandidateOverlayPositivePathValidation() async throws {
        guard stagingValidationFlagEnabled("SCOUTCAPTURE_RUN_26P_STAGING_CANONICAL_OVERLAY_VALIDATION") else {
            throw XCTSkip("Set SCOUTCAPTURE_RUN_26P_STAGING_CANONICAL_OVERLAY_VALIDATION=1 to run hosted staging canonical candidate overlay validation.")
        }
        let serviceKey = try stagingValidationHeaderValue("SCOUTCAPTURE_26P_STAGING_SERVICE_ROLE_KEY")
        let root = try makeTempStorageRoot()
        let store = LocalStore(testStorageRootURL: root)
        let orgID = UUID()
        _ = try store.createOrganization(Organization(id: orgID, name: "Phase 2C-26P Staging Org"))
        let property = try store.createProperty(Property(
            id: UUID(),
            orgId: orgID,
            name: "Phase 2C-26P Staging Canonical Overlay Validation"
        ))
        let session = try store.upsertSession(Session(
            id: UUID(),
            propertyID: property.id,
            startedAt: Date(timeIntervalSinceReferenceDate: 22_000),
            status: .completed,
            endedAt: Date(timeIntervalSinceReferenceDate: 22_600),
            exportedAt: nil,
            isSealed: true,
            firstDeliveredAt: nil,
            reExportExpiresAt: nil
        ...
        let shots = [
            makeShot(propertyID: property.id, sessionID: session.id, filename: "phase-2c-26p-1.jpg", localReference: false, remoteMetadata: false),
            makeShot(propertyID: property.id, sessionID: session.id, filename: "phase-2c-26p-2.jpg", localReference: false, remoteMetadata: false)
        ]
        try saveMetadata(store: store, property: property, session: session, orgID: orgID, shots: shots)
        ...
        let beforeOverlay = AppState.buildCanonicalCandidateOverlayTestOnly(
            targetClassification: .approvedStaging,
            canonicalDiagnostics: beforeDiagnostics,
            parityReport: beforeReport,
            candidateDiagnostics: beforeCandidate
        )
        XCTAssertFalse(beforeCandidate.allowed)
        XCTAssertFalse(beforeOverlay.allowed)
        XCTAssertNil(beforeOverlay.overlay)
        XCTAssertTrue(beforeOverlay.activeSourceRemainsLocal)
        XCTAssertTrue(beforeOverlay.localFallbackRetained)
            ...
            case .observation:
                try await self.insertStagingRow(
                    ...
                    payload: [
                        ...
                        "session_id": session.id.uuidString.lowercased(),
                        "shot_id": shots.first?.shotID.uuidString.lowercased() ?? NSNull(),
                        "category": "phase_2c_26p",
                        "status": "open",
                        "title": "Phase 2C-26P validation observation",
                        "detail": "Hosted staging canonical candidate overlay validation"
                    ],
                    serviceKey: serviceKey
                )
                return AppState.NormalizedBackfillEntityResult(kind: kind, attemptedCount: count, upsertedCount: 1, skippedCount: max(0, count - 1), failedCount: 0, message: "observation_row_inserted")
        ...
        let afterOverlay = AppState.buildCanonicalCandidateOverlayTestOnly(
            targetClassification: .approvedStaging,
            canonicalDiagnostics: afterDiagnostics,
            parityReport: afterReport,
            candidateDiagnostics: afterCandidate
        )
        ...
        XCTAssertTrue(afterCandidate.localFallbackAvailable)
        XCTAssertFalse(afterCandidate.productionWideCanonicalReadsEnabled)
        XCTAssertTrue(afterOverlay.allowed)
        XCTAssertNotNil(afterOverlay.overlay)
        XCTAssertEqual(afterOverlay.overlay?.source, "remote_normalized_candidate_with_local_fallback")
        XCTAssertEqual(afterOverlay.overlay?.propertyID, property.id)
        XCTAssertEqual(afterOverlay.overlay?.sessionID, session.id)
        XCTAssertEqual(afterOverlay.overlay?.remoteShotCount, afterCounts.shotRows)
        XCTAssertEqual(afterOverlay.overlay?.remoteIssueObservationCount, afterCounts.observationRows)
        XCTAssertEqual(afterOverlay.remoteCandidateRowCounts["remote_shots"], afterCounts.shotRows)
        XCTAssertEqual(afterOverlay.remoteCandidateRowCounts["remote_issue_observations"], afterCounts.observationRows)
        XCTAssertEqual(afterOverlay.overlay?.activeSource, "local")
        XCTAssertEqual(afterOverlay.overlay?.fallbackSource, "local")
        XCTAssertTrue(afterOverlay.localFallbackRetained)
        XCTAssertTrue(afterOverlay.activeSourceRemainsLocal)
        XCTAssertTrue(afterOverlay.rollbackAvailable)
        XCTAssertTrue(afterOverlay.productionBlocked)
        XCTAssertTrue(afterOverlay.noBehaviorChangedText.contains("production canonical reads remain disabled"))

        let productionOverlay = AppState.buildCanonicalCandidateOverlayTestOnly(
            targetClassification: .approvedProductionValidation,
            canonicalDiagnostics: afterDiagnostics,
            parityReport: afterReport,
            candidateDiagnostics: afterCandidate
        )
        XCTAssertFalse(productionOverlay.allowed)
        XCTAssertNil(productionOverlay.overlay)
        XCTAssertTrue(productionOverlay.blockedReason?.contains("production_canonical_candidate_overlay_blocked") == true)
        XCTAssertTrue(productionOverlay.productionBlocked)

        print("[Phase2C26P] staging_url=\(hostedStagingURL)")
        print("[Phase2C26P] property_id=\(property.id.uuidString)")
        print("[Phase2C26P] session_id=\(session.id.uuidString)")
        print("[Phase2C26P] before result=\(beforeDiagnostics.result.rawValue) candidate_allowed=\(beforeCandidate.allowed) overlay_allowed=\(beforeOverlay.allowed) completeness=\(beforeReport.parityCompletenessScore) missing_children=\(beforeReport.missingChildCount) remote_shots=\(beforeCounts.shotRows) remote_observations=\(beforeCounts.observationRows)")
        print("[Phase2C26P] replay allowed=\(replay.allowed) executed=\(replay.executedEntityCount) skipped=\(replay.skippedEntityCount) failed=\(replay.failedEntityCount)")
        print("[Phase2C26P] after result=\(afterDiagnostics.result.rawValue) recommendation=\(afterDiagnostics.canonicalRecommendation) completeness=\(afterReport.parityCompletenessScore) missing_children=\(afterReport.missingChildCount) remote_shots=\(afterCounts.shotRows) remote_observations=\(afterCounts.observationRows)")
        print("[Phase2C26P] candidate allowed=\(afterCandidate.allowed) effective_source=\(afterCandidate.effectiveSourceRecommendation) local_fallback=\(afterCandidate.localFallbackAvailable) production_wide=\(afterCandidate.productionWideCanonicalReadsEnabled)")
        print("[Phase2C26P] overlay built=\(afterOverlay.overlay != nil) allowed=\(afterOverlay.allowed) source=\(afterOverlay.overlay?.source ?? "not_built") active_source=\(afterOverlay.overlay?.activeSource ?? "local") fallback=\(afterOverlay.overlay?.fallbackSource ?? "local") production_blocked=\(afterOverlay.productionBlocked) shots=\(afterOverlay.overlay?.remoteShotCount ?? 0) observations=\(afterOverlay.overlay?.remoteIssueObservationCount ?? 0)")
    }
}
